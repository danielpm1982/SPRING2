...
as username and roles - authorities (see form.jsp), and also to generate specific jsp code according
to each authorized user role.
All other request mapping paths here, other than login, has the authorization conditioned not
only to the authentication of the user but also to the authorization (or not) of each role, 
