...
/*
This class also configures the SecurityDataSource for later use at MyWebSecurityConfigurerAdapter class
in order to store and retrieve registered user data to/from databases, instead of to/from program memory.
...
store in memory the properties file values and turn them available to objects. A SecurityDataSource bean
create method has been defined, with a ComboPooledDataSource being instantiated and set with the
properties file values, regarding jdbc and connection pool configurations, and then having that object
