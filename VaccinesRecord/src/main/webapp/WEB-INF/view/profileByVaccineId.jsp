...
<html>
    ...
    <body>
		...
		<div class="welcome">
			<p>Profile of Vaccine (ID = ${vaccine.vaccineId}):</p>
			<c:if test="${tempPhotoFileName != null}">
				<img src="${pageContext.request.contextPath}/temp/${tempPhotoFileName}" class="img10" id="img"/><br>
			</c:if>
			<p>Vaccine Id: ${vaccine.vaccineId}</p>
			<p>Name: ${vaccine.name}</p>
			<p>Trade Name: ${vaccine.tradeName}</p>
			<p>Abbreviation: ${vaccine.abbreviation}</p>
			<p>Manufacturer: ${vaccine.manufacturer}</p>
			<p>Route Type: ${vaccine.typeRoute}</p>
			<p>Approval Date: ${vaccine.approvedDate}</p>
			<p>Lot Number: ${vaccine.lotNumber}</p>
			<p>Expiration Date: ${vaccine.expirationDate}</p>
			<p>Comments: ${vaccine.comments}</p>
			<p>Vaccine Administrations:</p>
			<c:forEach items="${vaccine.vaccineAdministration}" var="vaccineAdministration" varStatus="status">
				<p>${status.count}: ${vaccineAdministration}</p>
			</c:forEach>
		</div>
